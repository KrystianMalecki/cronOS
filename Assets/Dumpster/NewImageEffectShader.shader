...
Shader "Custom/NewImageEffectShader"
{
    Properties
    {
        ...
        _AltTex("AltTexture", 2D) = "white" {}
        [Toggle]_UseAlt("UseAlt", Float) = 1
        [Header(Curvature)]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float3 col = tex2D(_MainTex, uv) * (1 - _UseAlt) + _UseAlt * tex2D(_AltTex, uv);
                ...
                if (_UseConvergence)
                {
                    ...
                    // Primary abberation
                    col.r = (
                        tex2D(_MainTex, float2(uv.x + abberationA, uv.y + abberationA)) * (1 - _UseAlt) +
                        _UseAlt * tex2D(_AltTex, float2(uv.x + abberationA, uv.y + abberationA))
                    ).x;
                    col.g = (
                        tex2D(_MainTex, float2(uv.x, uv.y - abberationA * 2)) * (1 - _UseAlt) +
                        _UseAlt * tex2D(_AltTex, float2(uv.x, uv.y - abberationA * 2))
                    ).y;
                    col.b = (
                        tex2D(_MainTex, float2(uv.x - abberationA * 2, uv.y)) * (1 - _UseAlt) +
                        _UseAlt * tex2D(_AltTex, float2(uv.x - abberationA * 2, uv.y))
                    ).z;
                    ...
                    col.r += d_str *
                        (tex2D(_Tex, d_offset * float2(0.025, -0.027) + float2(uv.x + abberationA, uv.y + abberationA))
                            *
                            (1 - _UseAlt) +
                            tex2D(_Tex, d_offset * float2(0.025, -0.027) + float2(
                                      uv.x + abberationA, uv.y + abberationA)) *
                            1
                        )
                        .x;
                    col.g += d_str * (tex2D(
                            ...
                            * (1 - _UseAlt) +
                            tex2D(
                                _Tex, d_offset * float2(0.022, -0.02) + float2(uv.x, uv.y - abberationA * 2)) * 1
                        )
                        .y;
                    col.b += d_str * (tex2D(
                        ...
                        * (1 - _UseAlt) +
                        tex2D(
                            _Tex, d_offset * float2(0.02, -0.018) + float2(uv.x - abberationA * 2, uv.y)) * 1
                    ).z;
                }
